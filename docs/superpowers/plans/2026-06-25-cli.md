...
> **Lower-churn alternative:** skip the move and have the CLI import `config_from_env` /
> `build_embedder` / `build_store` / `check_model_match` directly from `delfos.mcp.config`,
> adding only the `PlannerConfig`/`build_planner` piece in a new module. Choose this if you
> want zero edits to the MCP package; the rest of the plan is unchanged except import paths.
...
- Test: `tests/cli/__init__.py`, `tests/cli/test_render.py`, `tests/cli/test_parser.py`, `tests/cli/test_commands.py`
...
Create `tests/cli/test_commands.py` (integration: real temp store + FakeEmbedder, no network):
...
from tests.reconstruct.conftest import EMB_DIM, EMB_MODEL, FakeEmbedder
...
def test_index_then_status_reflects_written_files(tmp_path: Path) -> None:
    ...
    # FakeEmbedder returns a fixed vector for any cue text it is asked to embed.
...
> **Note on `FakeEmbedder`:** `tests/reconstruct/conftest.py` already defines it for keyed
> lookups. If it raises on unknown text, extend it (or add a `FixedEmbedder` in
> `tests/cli/conftest.py`) so `run_index` can embed arbitrary cue strings. Confirm the
> exact shape when implementing.
...
- Test: `tests/cli/test_render.py` (extend), `tests/cli/test_commands.py` (extend)

**Interfaces:**
- `render_search(cues: list[CueNode]) -> str`
- `render_reconstruct(contents: list[ContentNode]) -> str`
- New subparsers: `search <query> [-k N] [--index-path]`, `reconstruct <query> [--budget N] [-k N] [--index-path]`, `serve`.
...
then calls a `run_search(query, service)` helper and asserts the rendered output names the
seeded cue. (`reconstruct` is exercised by the existing planner tests + the smoke harness;
the CLI wiring is covered by the boot smoke in Step 4 rather than a network test.)
...
In `delfos/cli/app.py`, register the new subparsers in `build_parser` (`search`,
`reconstruct` with `--budget` default 3 and `-k` default 5, and a no-arg `serve`), add a
`run_search(query, k, service)` helper, and extend `main`'s dispatch:
...
tests over a temp `NativeGraphStore` with `FakeEmbedder`; no command test requires a
network. Argparse covered directly. ✓

**Placeholder scan:** the only open item is confirming `FakeEmbedder`'s behavior on
arbitrary text (flagged in Task 2 Step 1); everything else is concrete. ✓
